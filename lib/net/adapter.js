;(function (global) {
	...
	const RTCSessionDescription = global.RTCSessionDescription
	const RTCIceCandidate = global.RTCIceCandidate
	let getUserMedia = null
	let createIceServer = null
	...
	if (navigator.mozGetUserMedia) {
		...
		createIceServer = function (url, username, password) {
			...
			if (url_parts[0].indexOf('stun') === 0) {
				...
				url_parts[0].indexOf('turn') === 0 &&
				(url.indexOf('transport=udp') !== -1 || url.indexOf('?transport') === -1)
			) {
		...
		createIceServer = function (url, username, password) {
			...
			if (url_parts[0].indexOf('stun') === 0) {
				...
				if (webrtcDetectedVersion < 28) {
					...
					const url_turn_parts = url.split('turn:')
